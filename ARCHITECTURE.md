...
- **`amplify/backend/api/bigbet/`**: AppSync + DynamoDB (Amplify GraphQL transformer)
...
- **`amplify/backend/function/`**: Lambda functions
  ...
  - **`bigbetAuthPostConfirmation/`**: Cognito post-confirm trigger that creates `UserState` for a new user
...
- **`amplify/backend/custom/`**: custom CloudFormation stacks (Amplify “custom resources”)
  ...
  - **`guessSettlementScheduler/`**: provisions EventBridge Scheduler resources:
    - Schedule group `bigbet-guess-settlements-{env}`
